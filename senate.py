...
senate_sheet = page.worksheet('senate')

# Get existing data (to avoid inserting a row that is already scraped)
records = senate_sheet.get_all_records()
...
# todo: point to your own download folder
download_folder = '/Users/wboag/Downloads'
senate_downloads = glob.glob('%s/search_results_*.csv' % download_folder)
most_recent_download = sorted(senate_downloads)[-1]
...
senate_df = pd.read_csv(most_recent_download)
...
columns = senate_sheet.row_values(1)
print columns

inserted_count = 0
for _,row in senate_df.head(n=50)[::-1].iterrows():
    ...
    # insert row (TODO: doesnt work beyond 50 inserts)
    senate_sheet.insert_row(out, 2)
